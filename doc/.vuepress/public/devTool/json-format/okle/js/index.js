...
new Vue({
  ...
  data: {
    defaultResultTpl:
      '<div class="x-placeholder"><img src="./okle/images/json-demo.jpg" alt="json-placeholder"></div>',
    placeHolder: '',
    jsonFormattedSource: '',
    errorMsg: '',
    errorJsonCode: '',
    ...
    ;(this.placeHolder = this.defaultResultTpl),
      ...
      'chrome-extension:' === location.protocol &&
        chrome.runtime.onMessage.addListener(
          (e, t, o) => (
            'TAB_CREATED_OR_UPDATED' === e.type &&
              e.content &&
              e.event === location.pathname.split('/')[1] &&
              (editor.setValue(e.content || this.defaultResultTpl),
              this.format()),
            o && o(),
            !0
          )
  ...
  methods: {
    format: function () {
      ...
      try {
        ...
        null != s && ((t = s[1]), (e = s[2])), (o = JSON.parse(e))
      } catch (t) {
        try {
        ...
        this.errorMsg.length ||
          ...
          this.$nextTick(() => {
            ...
            this.autoDecode
              ...
              : o
              ? Formatter.formatSync(e)
              : Formatter.format(e),
              (this.jsonFormattedSource = e),
              null != t
                ? ((this.jfCallbackName_start = t + '('),
                  (this.jfCallbackName_end = ')'))
                : ((this.jfCallbackName_start = ''),
                  (this.jfCallbackName_end = ''))
          }),
      ...
      return (
        ...
        (this.jsonLintSwitch
          ? this.lintOn()
          : ((this.placeHolder =
              '<span class="x-error">' + this.errorMsg + '</span>'),
            !1))
      )
    },
    compress: function () {
    ...
    autoDecodeFn: function () {
      this.$nextTick(() => {
        localStorage.setItem(AUTO_DECODE, this.autoDecode), this.format()
      })
    },
    uniEncode: function () {
    ...
    urlDecode: function () {
      JsonEnDecode.urlDecodeByFetch(editor.getValue()).then((e) =>
        editor.setValue(e)
      )
    },
    ...
    lintOn: function () {
      return (
        ...
        !editor.getValue().trim() ||
          (this.$nextTick(() => {
            if (!this.jsonLintSwitch) return
            let e = JsonLint.lintDetect(editor.getValue())
            isNaN(e.line) ||
              (this.placeHolder =
                '<div id="errorTips"><div id="tipsBox">错误位置：' +
                (e.line + 1) +
                '行，' +
                (e.col + 1) +
                '列；缺少字符或字符不正确</div><div id="errorCode">' +
                e.dom +
                '</div></div>')
          }),
          !1)

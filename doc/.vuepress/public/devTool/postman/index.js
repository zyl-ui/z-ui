...
new Vue({
  ...
  methods: {
          ...
          case resp.target.DONE:
            try {
              result = JSON.stringify(
                JSON.parse(resp.target.responseText),
                null,
                4
              )
            } catch (e) {
    ...
    transParamMode() {
      if (this.paramMode === 'kv') {
        ...
        try {
          ...
          this.paramContent = Object.keys(obj)
            .map((k) => {
              let v = JSON.stringify(obj[k]).replace(/"/g, '')
              return `${k}=${v}`
            })
    ...
    didFormat: function (sortType) {
      ...
      if (sortType !== 0) {
        let jsonObj = JsonABC.sortObj(
          JSON.parse(this.originalJsonStr),
          parseInt(sortType),
          true
        )
        source = JSON.stringify(jsonObj)
    ...
    setDemo: function (type) {
      if (type === 1) {
        this.urlContent =
          'https://mock.apifox.cn/m1/1049320-0-default/getUserList?pageNum=1&pageSize=10'
        this.methodContent = 'GET'
        // this.paramContent = 'pageNum=1&pageSize=10'
      } else {
        this.urlContent =
          'https://mock.apifox.cn/m1/1049320-0-default/getUserList'
